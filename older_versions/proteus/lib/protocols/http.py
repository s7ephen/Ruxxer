...
class Header(Dataflow):
    ...
    def addAccept_Encoding(self, val=None):
        """
            HTTP Header Field
            Type: Request
            Usage: [HeaderInstance].addAccept_Encoding("[encoding string]")
        """
        if not issubclass(val.__class__, StringPDT):
            if issubclass(val.__class__, str):
                self.acceptEncodingValue = StringPDT(val)            
                self.add_stage(self.acceptEncodingLabel, self.acceptEncodingValue)       
        elif issubclass(val.__class__, StringPDT):
            self.add_stage(self.acceptEncodingLabel, val)
